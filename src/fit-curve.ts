 ...
 * Modified to also return last error index.
 */
 ...
 * fitCurve([{ x: 0, y: 0 }])
 */
export function fitCurve(points: Points, maxError: number): [Array<Curve>, number] {
    if (points.length < 2) {
        return [[], 0];
    }
...
function fitCubic(
    ...
    let [bezCurve, maxError, splitPoint] = generateAndReport(
        points,
        u,
        u,
        leftTangent,
    ...
    if (maxError < error * error) {
        ...
        for (let i = 0; i < MaxIterations; i++) {
            ...
            if (maxError < error) return [[bezCurve], 0];
    ...
    let lastIndex = 0;
    const [l, i] = fitCubic(points.slice(0, splitPoint + 1), leftTangent, toCenterTangent, error);
    for (const curve of l) {
        beziers.push(curve);
    }
    lastIndex = Math.max(lastIndex, i);
    const [r, j] = fitCubic(points.slice(splitPoint), fromCenterTangent, rightTangent, error);
    for (const curve of r) {
        beziers.push(curve);
    }
    lastIndex = Math.max(lastIndex, j + splitPoint);
    return [beziers, lastIndex];
}
...
export function bezierQ(ctrlPoly: Curve, t: number): Point {
    const tx = 1.0 - t;
